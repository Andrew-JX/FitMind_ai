...
### Remaining risks
- The summary DTOs are intentionally frontend-local to preserve the five-file batch limit, so future shared-contract cleanup may still be worthwhile once file-count constraints are lifted.
- The panel styling approximates `docs/UI_SPEC.md` using local inline styles because the repo has not yet landed the dedicated token files described in the spec.
- This batch keeps a fixed client-computed 30-day window and does not yet expose date controls.
## 2026-04-30 Phase 2.0 Batch 3 - Backend Exercise Progress API

### Completed work
- Added an authenticated `GET /api/training/exercise-progress` backend endpoint behind the existing auth middleware.
- Added query validation for `exercise_id`, `start_date`, and `end_date`, including UUID enforcement, `YYYY-MM-DD` enforcement, and the invalid-range guard when `end_date` is earlier than `start_date`.
- Added a dedicated repository query layer for deterministic exercise progress aggregation using only the existing `workouts`, `sets`, and `exercises` tables.
- Added a service layer that shapes overall totals, per-session rollups, evidence ids, and deterministic calculation rules for the new endpoint.
- Kept workout CRUD contracts and database schema unchanged.
...
- `pnpm --filter @fitmind/server type-check`
- `pnpm lint`
- Manual HTTP smoke if local server is available
## 2026-04-30 Phase 2.0 Batch 3.1 - Exercise Progress Regression Smoke/Test

### Completed work
- Added a dedicated `server/scripts/exercise-progress-api-smoke.ts` regression smoke in the same style as the existing workout and training summary smoke scripts.
- Covered auth failure, missing `exercise_id`, invalid `start_date`, invalid date range, empty-range behavior, populated totals, session rollups, unrelated-exercise exclusion, cross-user isolation, and deletion cleanup for `GET /api/training/exercise-progress`.
- Verified the new endpoint through real HTTP calls that create and delete workouts through the existing workout contracts rather than bypassing the app layers.
- Kept the batch backend-only with no schema or contract changes.
...
## 2026-04-30 Phase 2.0 Batch 4 - Frontend Readonly Exercise Progress Panel

### Completed work
- Added a client-side exercise progress API wrapper on top of the existing shared HTTP client for `GET /api/training/exercise-progress`.
- Added a dedicated readonly exercise progress panel with local request state, client-computed last-30-days range, and no-request behavior when no exercise is selected.
- Updated the summary panel so top exercises can be selected as clickable progress targets while preserving the existing summary refresh and readonly totals behavior.
- Updated the authenticated app shell to own exercise-progress selection state and a progress refresh signal that changes after successful create/delete operations when an exercise is selected.
- Kept the batch inside the requested five handwritten files with no backend, schema, route, or shared-contract changes.
...
- `pnpm --filter @fitmind/client type-check`
- `pnpm lint`
- Manual browser smoke if local client/server are available
## 2026-04-30 Phase 2.0 Batch 5 - Closeout Docs and Manual Smoke Notes

### Completed work
- Added `docs/calculation-layer.md` to document the deterministic-before-AI rationale for Phase 2.0.
- Documented the current calculation endpoints, their evidence fields, inclusive calendar inputs, half-open timestamp filtering, null-safe aggregation conventions, and the Epley 1RM approximation.
- Captured explicit user-isolation behavior and clarified that future tool-calling relevance is only a downstream possibility, not a feature added in this phase.
- Kept Phase 2.0 non-goals explicit so the closeout stays aligned with the current repo constraints and the intentional scope boundary.
...
- `pnpm --filter @fitmind/server exec tsx scripts/training-summary-api-smoke.ts`
- `pnpm --filter @fitmind/server exec tsx scripts/exercise-progress-api-smoke.ts`
- Manual browser smoke only if actually completed
