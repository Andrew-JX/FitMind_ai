...
## 2026-08-15 — AI 周计划 V2 与本地真实数据链路收口

- 实现候选 `643daaa`（baseline `2c00a38508ea968d9dbbc12ec5b8d3199bac6c73`）：将“下周训练草案”和“给我下周计划”统一到同一份一页式设置/生成入口；临时训练天数、时长、器械、状态和重点部位只作用于本次生成，不覆盖运动员档案。
- 计划输出改为柔性“训练日 1..N”结构；生成器按器械和已记录伤病过滤动作，保留扁平动作集合兼容既有依从度。草案支持删除、合规替换及组数/次数/休息编辑，接受时提交编辑后的结构。
- 用户本地验收追加三项 UI 收口：旧计划快照缺少依从度不再黑屏（应用错误边界 + 安全默认值）；补齐境外数据存储同意改为直接主按钮及登录页式加载动效；训练页本周计划可原地展开/收起，展示训练日、动作、组次和休息。助手快入口文案同步区分“回顾本周训练”和“制定下周计划”。
- `.neon.tech` 数据库地址改用 Neon 官方 pg-compatible WebSocket Pool，非 Neon 地址保持 `pg`；本地真实 API 已读取到 43 个动作和 17 个肌群，解决代理/Fake-IP 环境下 Neon 原生 TCP 不通的问题。
- 最终验证：`pnpm verify` 通过（121 个 Vitest 文件、960 条断言及 5 条 monitor 断言）；客户端 production build 通过；已在登录后的真实本地训练页手动验证“展开查看本周计划”可展示四个训练日与动作详情。补同意分支未为取证而改写真实账户状态，故只由源码、构建和自动化回归覆盖。未执行部署；推送后的远端 SHA 在本次收尾提交后单独核对。

## 2026-08-15 — P2 状态收口

- roadmap E2「核心 UI 空态、错态、加载态、断网、慢响应和边界数据走查」在既有完整状态矩阵证据上，加入本周计划异常快照、补同意加载态、训练页展开和真实数据连接的用户验收收口，标记完成。
- repository 全面 TypeScript 化沿用 2026-08-14 独立整合收口记录；本轮全仓 `pnpm verify` 再次通过，未引入 JavaScript 源码回退。
