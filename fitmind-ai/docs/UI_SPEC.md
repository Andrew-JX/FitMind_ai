...
| Tab 顺序 | ① 训练（dumbbell）② 历史（clock）③ 个人（user）；AI 助手保留为独立圆形入口 |
...
**结构**：标题 → 说明段（含隐私政策链接）→ 剩余项计数（多于一项时）→ 同意声明全文（含政策
版本号，作为文本直接展示）→ 主按钮「我已阅读并同意，继续」（点击即同意，无前置勾选）→ 次
按钮「暂不同意，先退出登录」→ 危险按钮「不同意，并删除我的账号与全部数据」（红色描边）→
脚注。

（2026-09 交互简化：旧版的「同意勾选 + 未勾选禁用主按钮」已移除——声明全文直接可见，
点击主按钮本身就是同意的动作。`sensitive_health_data` 类型的同意会额外提供一个中间出口
「不同意，请删除全部健康数据（保留账号与训练记录）」，因为为一条同意失去全部训练史不成比例。）
...
**删除是二次确认加密码**：第一次点展开红框列出会被删掉的东西并写明不可撤销（含托管商备份
保留期的如实说明），随后必须输入当前密码，「确认删除」才发请求。失败时明说「你的数据没有被
改动」——删除失败最怕的是用户以为删掉了。服务端会再次校验密码，前端弹窗保护不了直接调 API
的人。
...
快速统计下方使用 3 列等宽入口，顺序固定：

| 入口 | 行为 |
| --- | --- |
| 语音记录训练 | 打开语音录入，实时显示识别文字；示例为“我今天练了胸，杠铃卧推 80 公斤，5 组，每组 8 次” |
| 文本录入训练 | 打开空文本框，允许输入或粘贴训练笔记；不启动语音识别 |
| 手动记录训练 | 直接打开空白训练编辑器 |

语音与文本入口必须复用同一条 `parseWorkoutIntake → 动作歧义确认 → TrainingSessionComposer`
链路。两者只在采集文字的方式上不同，不得维护两套动作识别、确认或保存逻辑。示例只作为
`placeholder`，不能预填为真实输入值。
...
底部「历史」是训练回顾的统一入口。页面顶部使用居中的双段切换：`历史 | 分析`，首次进入
默认选中「历史」。切换只改变当前内容区，不卸载另一侧状态；因此日历月份、列表展开状态、
分析范围和已选动作不会因为来回切换而丢失。训练首页不再重复展示训练记录。

#### 4.2.1 历史视图：训练日志列表

列表标题行："训练日志"（15px/700）+ 条数标签（11px/`--fm-tx3`）。

**日历视图**：固定 7 列、周日开头，月网格同时显示相邻月份日期。训练日单元格按
“日期 / 当日总容量（公斤）/ 每次训练备注”纵向排列；总容量汇总当天所有训练的组容量，
备注每次训练占一行并使用单行省略，完整内容保留在 `title` 与可访问名称中。未填写备注显示
弱提示「未填写备注」。点击训练日仍筛选并展示当天的 WorkoutCard，再次点击取消筛选；无训练
日期不可点击。旧 API 尚未返回容量时，客户端仅对当前可见月份的训练按需读取详情并回算容量；
回算完成前或详情失败时显示「—」，不得误报为 0。
...
**训练页镜像卡片（TrainingPlanCard）**：训练页保持紧凑的周期、依从度和进度摘要，并提供明确的「展开查看本周计划 / 收起本周计划」按钮。展开后在原位展示所有柔性训练日、每个动作的名称、组数、次数和组间休息；不跳转助手页，也不新建第二份计划。异常/旧快照缺少依从度时使用安全默认值，页面由应用错误边界兜底而非黑屏。
...
- 无效凭据明确提示“账号或密码错误，请重新输入”，不得显示后端英文错误。
- 服务端或网络故障使用可行动的暂时性提示，不得冒充凭据错误。
- 注册同意项使用 18px 原生复选框，选中态为低强调灰色勾；整行标签保持可点击，长文案时复选框顶部对齐。
- 法律页版本说明使用正式、简洁的产品措辞；说明记录版本与重大变更重新征求同意即可，不使用内部演示式解释。

## 14. 动作详情关闭入口

- `ExerciseDetailSheet` 标题区右侧显示 32×32px 的 `x` IconButton，使用次级表面与普通边框。
- 按钮可访问名称为“关闭”；点击后调用当前详情的 `onClose`，返回动作列表，不改变筛选和搜索状态。
- 保留点击遮罩关闭的原有行为；显式按钮是移动端长内容滚动后的可发现出口。
