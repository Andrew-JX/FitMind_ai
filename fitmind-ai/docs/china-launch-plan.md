...
删除由用户在 Neon 控制台执行（本仓库没有生产库权限）。所有 7 张引用 `users` 的表
都是 `ON DELETE CASCADE`，`sets → workouts`、`chat_messages → chat_sessions` 也是，
所以删 `users` 一行即可级联干净。**语句必须自带守卫**：`DELETE FROM users WHERE
