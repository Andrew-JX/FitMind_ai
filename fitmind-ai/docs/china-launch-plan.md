...
`VOYAGE_API_KEY` / `GROQ_API_KEY` / `ANTHROPIC_API_KEY` 一律留空。代码保留不影响（都有 fail-safe 分支）。**不给 key = 没有个人信息出境**，这比在文档里声明「我们不出境」硬得多。
...
### 6.1 开放注册已经是计划中的事（2026-08-02 用户确认）

境内实例的邀请制是**阶段性**的，不是终局：先小范围用，之后要开放注册登录。
也就是说 §3.1 的豁免**从一开始就是有期限的**，第一条触发条件迟早会成立。

由此产生的两条约定：

- **法律文案不写死运营状态。** 用户协议不再声称「以邀请制提供、不开放公开注册」——
  那句话在开放注册当天就变成假话，而没有任何流程会提醒去改它。协议改为说明
  「开放方式随阶段调整，以注册页实际是否可用为准」，隐私政策则按实例分别说明数据
  存放地与第三方调用。**凡是活在配置里、会随阶段变的事实，不写进法律文案。**
