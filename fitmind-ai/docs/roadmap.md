...
**2026-06-17 更新**：§8 Slice 1–7 + 前端 FE-1~4 均已落地，产品功能层已相对完整。下一档推荐主线是 **§8.1 理解层升级（real-LLM seam）**——把对话路由 / 录入解析这两个"听懂用户"的环节从死正则换成真实（免费 Groq）模型，同时治"对话死板"和"录入变组识别不了"两个长期体验痛点，并让 Slice 1/2 护栏真正承重。建议顺序见 §8.1 末。
...
### 8.1 理解层升级（real-LLM seam）— 2026-06-17 与用户共定方向
...
### 8.2 优化总 Slice（执行总路线，2026-06-17 与用户共定）
...
| | A2 = **Slice 11a** | 对话"不死"纯确定性止血：`unsupported` 走 RAG 检索 + "你是不是想问 X" 澄清 + 扩同义词 | 无 | 🟢 待做（仅缓解） |
