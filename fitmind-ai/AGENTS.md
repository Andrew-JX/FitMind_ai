...
- **多步 Agent（Phase 6.0 ✅）**：`next_week_plan` intent 走 `server/src/services/agent/` 的确定性 ReAct 规划器（查容量→找弱项→查进展→检索知识→生成草案），发 `agent_step_*` SSE 事件 + `state:"planning"`，`agent_trace` 进 `structured_output` 持久化；前端 `AssistantAgentTrace.tsx` 在消息气泡里渲染 thought→action→observation 时间线。synthesis 还产出确定性可执行下周草案 `plan`（动作×组×次×目标重量，`next-week-plan-generator.ts`，结构化不内联、先不落库）进 structured_output。
